...
for el in data:
    if el['daten_stand'] is None:
        continue
    el['datenbestand'] = datetime.datetime.utcfromtimestamp(int(el['daten_stand'] / 1000))
    el['betten_gesamt'] = el['Betten_gesamt']
    del el['daten_stand']
    del el['Betten_gesamt']
    
    entries.append(el)
    
logger.debug('current count for histoic bed capacity data: %s', len(entries))
...
try:  
    ...
    if last_update is not None and 0 <= ((current_update - last_update).total_seconds()) <= 2*60*60:
