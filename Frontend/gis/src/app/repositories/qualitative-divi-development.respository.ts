...
export class QualitativeDiviDevelopmentRepository {
  ...
  private getDiviDevelopmentCounties(refDate: Date = new Date(), dayThreshold: number = 5): Observable <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> {
    return this.cachedRepository.get <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> (`${environment.apiUrl}hospitals/development/landkreise`, this.prepareParams(refDate, dayThreshold));
  }
  ...
  private getDiviDevelopmentGovernmentDistricts(refDate: Date = new Date(), dayThreshold: number = 5): Observable < FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>> > {
    return this.cachedRepository.get <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> (`${environment.apiUrl}hospitals/development/regierungsbezirke`, this.prepareParams(refDate, dayThreshold));
  }
  ...
  private getDiviDevelopmentStates(refDate: Date = new Date(), dayThreshold: number = 5): Observable <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> {
    return this.cachedRepository.get <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> (`${environment.apiUrl}hospitals/development/bundeslaender`, this.prepareParams(refDate, dayThreshold));
  }
  ...
  public getDiviDevelopmentCountries(refDate: Date = new Date(), dayThreshold: number = 5): Observable <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> {
    return this.cachedRepository.get <FeatureCollection<MultiPolygon, AggregatedHospitalOut<QualitativeTimedStatus>>> (`${environment.apiUrl}hospitals/development/laender`, this.prepareParams(refDate, dayThreshold));
  }
  ...
  public getDiviDevelopmentSingleHospitals(refDate: Date = new Date(), dayThreshold: number = 5): Observable <FeatureCollection<Point, SingleHospitalOut<QualitativeTimedStatus>>> {
    return this.cachedRepository.get <FeatureCollection<Point, SingleHospitalOut<QualitativeTimedStatus>>> (`${environment.apiUrl}hospitals/development`, this.prepareParams(refDate, dayThreshold))
    // .pipe(
    //   map(d => {
    //     if(!refDate) {
    //       return d;
