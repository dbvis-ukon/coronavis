...
class AverageBardichteLayer extends Overlay {
    ...
    createOverlay() {
        ...
        // .domain expects an array of [min, max] value
        // d3.extent returns exactly this array
        const minMaxBars = d3.extent(this.featureCollection.features.map(d => (d.properties.num_bars / d.properties.population)));
        console.log(minMaxBars);
        const colorScale = d3.scaleSequential(d3.interpolateReds).domain(minMaxBars);
        ...
        const nameP = tooltip.append('p');
        const areaP = tooltip.append('p');
        const percentageP = tooltip.append('p');

        // create geojson layer (looks more complex than it is)
        const bardichteLayer = L.geoJSON(this.featureCollection, {
            style: (feature) => {
                return {
                    fillColor: colorScale((feature.properties.num_bars / feature.properties.population)),
                    weight: 2,
                    opacity: 1,
                    color: 'white',
                    dashArray: '3',
            ...
            onEachFeature: (feature, layer) => {
                layer.on({
                    ...
                    mouseover: (e: L.LeafletMouseEvent) => {
                        ...
                        tooltip.style('left', `${e.originalEvent.clientX + 25}px`);
                        nameP.text(`County: ${feature.properties.name}`);
                        areaP.text(`#Bars: ${feature.properties.num_bars} for ${feature.properties.population} people.`);
                        percentageP.text(`(${Math.round((feature.properties.num_bars / feature.properties.population) * 10000) / 100} %)`);
...
export { Overlay, LandkreisLayer, BardichteLayer, AverageBardichteLayer};
